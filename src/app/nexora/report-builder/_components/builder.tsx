...
import { ANALYTICS_RECORDS, MONTHLY } from "@/data/nexora/datasets";
...
export function ReportBuilder() {
  ...
  const renderVisual = () => {
    switch (chartType) {
      case "line":
        return <NexoraLineChart data={timeSeries} series={series} xKey="label" format={primaryFormat} />;
      case "area":
        return <NexoraAreaChart data={timeSeries} series={series} xKey="label" format={primaryFormat} />;
      case "composed":
        return (
          <NexoraComposedChart
            data={timeSeries}
      ...
      case "pie":
        return <NexoraPieChart data={grouped.map((row) => ({ name: String(row.name), value: Number(row.value) }))} />;
      case "donut":
        return (
          <NexoraPieChart data={grouped.map((row) => ({ name: String(row.name), value: Number(row.value) }))} donut />
        );
      case "scatter":
        return (
          <NexoraScatterChart
