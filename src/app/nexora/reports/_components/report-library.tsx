...
import { REPORTS, type ReportDefinition } from "@/data/nexora/content";
import { formatDate, formatNumber } from "@/data/nexora/format";
...
export function ReportLibrary() {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="flex min-w-0 flex-col">
            <Link
              href={`/nexora/reports/${row.original.id}`}
  ...
  return (
    <NexoraDataTable
      ...
      onRowClick={(report) => router.push(`/nexora/reports/${report.id}`)}
      toolbar={
        <>
