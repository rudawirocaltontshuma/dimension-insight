...
import { KPIS, REPORTS } from "@/data/nexora/content";
import { CUSTOMERS, EMPLOYEES, PRODUCTS } from "@/data/nexora/datasets";
import { formatCurrency } from "@/data/nexora/format";

import { ANALYTICS_MODULES, NEXORA_BASE, NEXORA_NAV } from "../_lib/navigation";
...
function buildEntries(): SearchEntry[] {
  const navigation: SearchEntry[] = [...NEXORA_NAV.flatMap((group) => group.items), ...ANALYTICS_MODULES].map(
    (item) => ({
      id: `nav-${item.href}`,
      group: "Navigation",
      label: item.title,
  ...
  const reports: SearchEntry[] = REPORTS.map((report) => ({
    ...
    href: `${NEXORA_BASE}/reports/${report.id}`,
  }));
  ...
  const kpis: SearchEntry[] = KPIS.map((kpi) => ({
    ...
    href: `${NEXORA_BASE}/kpis?kpi=${kpi.id}`,
  }));
  ...
  const analytics: SearchEntry[] = [
    { label: "Revenue Trend", href: `${NEXORA_BASE}/analytics/finance`, hint: "Trailing 24 month revenue series" },
    { label: "Sales by Region", href: `${NEXORA_BASE}/analytics/sales`, hint: "Regional revenue contribution" },
    { label: "Churn Analysis", href: `${NEXORA_BASE}/analytics/customers`, hint: "Churn and retention movement" },
    { label: "Fulfilment Mix", href: `${NEXORA_BASE}/analytics/operations`, hint: "Order fulfilment distribution" },
    { label: "Channel ROI", href: `${NEXORA_BASE}/analytics/marketing`, hint: "Return on marketing investment" },
    { label: "Headcount Trend", href: `${NEXORA_BASE}/analytics/workforce`, hint: "Headcount and hiring movement" },
    { label: "Data Explorer", href: `${NEXORA_BASE}/data-explorer`, hint: "Ad hoc dimension and metric analysis" },
